...
class MHA(tf.keras.layers.Layer):
    ...
    def __init__(
        ...
        self.in_proj = tf.keras.layers.Dense(units=(qkv_dim + self.mlp_dim), use_bias=qkv_proj_bias, kernel_initializer='glorot_uniform', **factory_kwargs)
        if self.d_conv > 0:
            self.conv1d = tf.keras.layers.Conv1D(
                filters=qkv_dim, 
                kernel_size=self.d_conv, 
                strides=1,
                padding='causal', # used to be "self.d_conv - 1" # EMRAN not sure about this one 
                groups=qkv_dim,
                kernel_initializer='glorot_uniform',
                **factory_kwargs
            )
        self.out_proj = tf.keras.layers.Dense(units=embed_dim, use_bias=out_proj_bias, kernel_initializer='glorot_uniform', **factory_kwargs)
    ...
    def _update_kvcache_attention(self, q, kv, inference_params):
        ...
        if (
            ...
            k, v = kv.unbind(dim=-3)
            k = tf.repeat_interleave(k, repeats=self.num_heads // self.num_heads_kv, axis=2)
            v = tf.repeat_interleave(v, repeats=self.num_heads // self.num_heads_kv, axis=2)
            return tf.nn.scaled_dot_product_attention(
    ...
    def call(self, x, inference_params=None):
        ...
        if self.mlp_dim > 0:
            ...
            x_mlp = x_mlp_up * tf.keras.activations.silu(x_mlp_gate)
        if self.d_conv > 0:
            ...
            if (inference_params is None or inference_params.seqlen_offset == 0):
                ...
                if inference_params is not None:
                    ...
                    # pad_len = self.d_conv - tf.shape(qkv_t)[-1]
                    qkv_t = tf.pad(qkv_t, paddings=[[0, 0]] * (len(qkv_t.shape) - 1) + [[self.d_conv - tf.shape(qkv_t)[-1], 0]])
            else:
                _, conv_state = inference_params.key_value_memory_dict[self.layer_idx]
                ...
                if causal_conv1d_update is None:
                    conv_state.copy_(tf.roll(conv_state, shifts=-1, axis=-1))  # Update state (B D W)
                    conv_state[:, :, -1] = qkv
                    qkv = tf.sum(conv_state * rearrange(self.conv1d.weight, "d 1 w -> d w"), axis=-1)  # (B D)
        ...
        if (
            ...
            if inference_params is None:
                ...
                k = tf.repeat_interleave(k, repeats=self.num_heads // self.num_heads_kv, axis=2)
                v = tf.repeat_interleave(v, repeats=self.num_heads // self.num_heads_kv, axis=2)
                context = tf.nn.scaled_dot_product_attention(
        ...
        if self.mlp_dim > 0:
            context = tf.concat([context, x_mlp], dim=-1)

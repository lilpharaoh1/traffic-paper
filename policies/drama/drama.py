...
from policies.drama.drama_catalog import DramaCatalog
from policies.drama.utils import do_symlog_obs
from policies.drama.utils.env_runner import DramaEnvRunner
from policies.drama.utils.summaries import (
...
class DramaConfig(AlgorithmConfig):
    """Defines a configuration class from which a Drama can be built.

    .. testcode::

        from ray.rllib.algorithms.drama import DramaConfig
        config = (
            DramaConfig()
    ...
    def __init__(self, algo_class=None):
        """Initializes a DramaConfig instance."""
        super().__init__(algo_class=algo_class or Drama)

        # fmt: off
        # __sphinx_doc_begin__

        # Drama specific settings:
        self.model_size = "D"
        ...
        # Reporting.
        # Drama is super sample efficient and only needs very few episodes
        ...
        # Override some of AlgorithmConfig's default values with Drama-specific
        # values.
        self.lr = None
        ...
        self.env_runner_cls = DramaEnvRunner
    ...
    def batch_size_B_per_learner(self):
        ...
        Needed by some of the Drama loss math."""
        ...
        **kwargs,
        ...
        Args:
            ...
            replay_buffer_config: Replay buffer config.
                Only serves in Drama to set the capacity of the replay buffer.
        ...
        if use_curiosity is not NotProvided:
            raise ValueError(
                "`DramaConfig.curiosity` is not fully supported and tested yet! "
    ...
    def validate(self) -> None:
        ...
        # Make sure, users are not using Drama yet for multi-agent:
        if self.is_multi_agent():
            raise ValueError("Drama does NOT support multi-agent setups yet!")

        # Make sure, we are configure for the new API stack.
        if not self._enable_new_api_stack:
            raise ValueError(
                "Drama must be run with `config.experimental("
        ...
        if self.num_learner_workers > 1 and (
            ...
            raise ValueError(
                ...
                "Drama to be able to split batches evenly across your Learner "
                "processes."
            )
        ...
        # Use Drama specific batch size settings.
        ...
        if self.replay_buffer_config.get("type") != "EpisodeReplayBuffer":
            raise ValueError(
                "Drama must be run with the `EpisodeReplayBuffer` type! None "
                "other supported."
            )
    ...
    def get_default_learner_class(self):
        if self.framework_str == "tf2":
            from policies.drama.tf.drama_tf_learner import (
                DramaTfLearner,
            )

            return DramaTfLearner
        else:
    ...
    def get_default_rl_module_spec(self) -> SingleAgentRLModuleSpec:
        if self.framework_str == "tf2":
            from policies.drama.tf.drama_tf_rl_module import (
                DramaTfRLModule,
            )

            return SingleAgentRLModuleSpec(
                module_class=DramaTfRLModule, catalog_class=DramaCatalog
            )
        else:
            raise ValueError(f"The framework {self.framework_str} is not supported.")
...
class Drama(Algorithm):
    """Implementation of the model-based Drama RL algorithm described in [1]."""

    # TODO (sven): Deprecate/do-over the Algorithm.compute_single_action() API.
    @override(Algorithm)
    def compute_single_action(self, *args, **kwargs):
        raise NotImplementedError(
            "Drama does not support the `compute_single_action()` API. Refer to the"
            " README here (https://github.com/ray-project/ray/tree/master/rllib/"
            "algorithms/drama) to find more information on how to run action "
    ...
    def setup(self, config: AlgorithmConfig):
        ...
        if self.config.framework_str == "tf2":
            self.workers.local_worker().module.drama_model.summary(expand_nested=True)
        ...
        while (
            ...
            with self._timers[LEARN_ON_BATCH_TIMER]:
                ...
                if self.config.report_images_and_videos:
                    report_predicted_vs_sampled_obs(
                        # TODO (sven): Drama is single-agent only.
    ...
    # TODO (sven): Remove this once Drama is on the new SingleAgentEnvRunner.
    @PublicAPI
    def __setstate__(self, state) -> None:
        """Sts the algorithm to the provided state

        Args:
            state: The state dictionary to restore this `Drama` instance to.

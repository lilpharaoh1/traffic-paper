...
from policies.drama.drama import DramaConfig
from policies.drama.drama_learner import DramaLearner
...
class DramaTfLearner(DramaLearner, TfLearner):
    """Implements Drama losses and gradient-based update logic in TensorFlow.

    The critic EMA-copy update step can be found in the `DramaLearner` base class,
    ...
    def postprocess_gradients_for_module(
        ...
        *,
        ...
        for optimizer_name, optimizer in self.get_optimizers_for_module(
            ...
            # Drama stats have the format: [WORLD_MODEL|ACTOR|CRITIC]_[stats name].
    ...
    def compute_gradients(
        ...
        **kwargs,
        ...
        # For Drama, we need to compute gradients over the individual loss terms
    ...
    def _compute_world_model_prediction_losses(
        ...
        *,
        ...
        Args:
            ...
            fwd_out: The `forward_train` outputs of the DramaRLModule.
        """
    ...
    def _compute_world_model_dynamics_and_representation_loss(
        self, *, config: DramaConfig, fwd_out: Dict[str, Any]
